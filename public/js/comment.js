...
export const comment = (() => {
    ...
    const edit = async (button) => {
        ...
        if (status?.code === 200) {
            ...
            ${
                document.getElementById(id).getAttribute("data-parent") ===
                "true"
                    ? `
            ...
            document
                .getElementById(`button-${id}`)
                .insertAdjacentElement("afterend", inner);
            document.getElementById(`form-inner-${id}`).value =
                status.data.comment;
        }
    ...
    const comment = async () => {
        ...
        await request(
            HTTP_GET,
            `/api/comment?limit=${pagination.getPer()}&page=${pagination.getNext()}`
        )
            .token(session.get("token"))
            .then((res) => {
                if (res.code !== 200) {
